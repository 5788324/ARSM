...
export default function WorkClient({ work }: { work: any }) {
  ...
  const trackToItem = (t: Track) => ({ id: t.id, title: t.title, trackNumber: t.trackNumber, workTitle: work.displayTitle as string, workId: work.id as string, coverPath: work.coverPath as string | null, streamUrl: `/api/tracks/${t.id}/stream`, durationSec: t.durationSec });
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 pb-24">
      ...
      <div className="mt-3 flex flex-col sm:flex-row gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            {work.workVAs?.length > 0 && <span className="text-sm text-zinc-500">声优: {work.workVAs.map((v: any) => <Link key={v.voiceActor.id} href={`/voice-actors/${v.voiceActor.id}`} className="text-blue-600 hover:underline dark:text-blue-400 mr-1">{v.voiceActor.name}</Link>)}</span>}
          </div>
            ...
            <div key={key} className="rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
              <button onClick={() => toggleGroup(key)} className="w-full flex items-center justify-between px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-900">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-500">{group.tracks.length} 首 · {fmt(gDur)}</span>
                </div>
              ...
              {!collapsed && (
                <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  {group.tracks.map((t) => {
                    ...
                    return (
                      <div key={t.id} className={`flex items-center gap-2 px-4 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-900 ${active ? 'bg-blue-50 dark:bg-blue-950' : ''}`}>
                        <button onClick={() => handlePlayAll([t])} className="text-sm">{active && playing ? '⏸' : '▶'}</button>
                        <span className="w-7 text-center text-xs text-zinc-400">{String(t.trackNumber).padStart(2, '0')}</span>
                        <div className="flex-1 min-w-0"><p className={`text-sm truncate ${active ? 'font-medium text-blue-700 dark:text-blue-300' : ''}`}>{t.title}</p></div>
                        {t.durationSec && <span className="text-xs text-zinc-400">{fmt(t.durationSec)}</span>}
      ...
      {/* Subtitle viewer */}
      {subViewer && <SyncedSubtitleViewer subId={subViewer.id} fileName={subViewer.name} onClose={() => setSubViewer(null)} />}
    </div>
  );
}
