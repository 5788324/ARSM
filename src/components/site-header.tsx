    ...
    <header className="border-b border-gray-200 bg-white/95 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        <div className="flex items-center gap-6 text-sm">
          <Link href="/" className="text-lg font-semibold text-black dark:text-white">ARSM</Link>
          <nav className="hidden items-center gap-4 md:flex">
            <Link href="/works" className={linkClass('/works')}>作品库</Link>
            <Link href="/favorites" className={linkClass('/favorites')}>收藏</Link>
        ...
        <div className="flex items-center gap-3 text-sm text-gray-600">
          ...
          <button type="button" onClick={() => signOut({ callbackUrl: '/login' })} className="rounded border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800">退出</button>
        </div>
      </div>
    </header>
  );
