...
export function SyncedSubtitleViewer({ subId, fileName, onClose }: SyncedSubtitleProps) {
    ...
    fetch(`/api/subtitles?id=${activeSubId}`)
      ...
      .then(data => {
        if (data.ok) {
          ...
          setHasTiming(data.data.hasTiming);
        }
        setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    const el = document.querySelector(`[data-subcue-time="${Math.floor(currentTime)}"]`);
    if (el) el.scrollIntoView({ block: 'center', behavior: 'smooth' });
  }, [currentTime, cues, hasTiming]);

  const findActiveIdx = () => {
    if (!cues) return -1;
    return cues.findIndex(c => c.start <= currentTime && c.end >= currentTime);
  };
        ...
        <div ref={scrollRef} className="flex-1 overflow-auto px-6 py-4">
          {hasTiming && cues ? (
            <div className="space-y-1">
              {cues.map((cue, i) => {
                const active = findActiveIdx() === i;
                return (
                  <div key={i} data-subcue-time={Math.floor(cue.start)}
                    onClick={() => seek(cue.start)}
                    className={`cursor-pointer rounded px-3 py-1.5 text-sm leading-relaxed transition-colors ${active ? 'bg-blue-100 text-blue-900 dark:bg-blue-900 dark:text-blue-200 font-medium' : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800'}`}>
